...
There are also 9 people with 2 or more tab-separated NWD ids. It is not clear which is the preferred one, so I will arbitrarily take the first. It is not enough people to warrant the merging complications that would arise from trying to consider them all.
```{r}
tmp <- fhs_query[, .SD, .SDcols=patterns('SAMPLE_ID')
][, names(.SD) := lapply(.SD, \(x) gsub('\t',' ',x))
] |> setnames(\(nm)  sub('.*\\\\(.*)\\\\$', '\\1', nm)) # Then, manually explored in VisiData

tmp[, .SD[[2]][grepl(' ',.SD[[2]])]] # Multiple NWD ids
