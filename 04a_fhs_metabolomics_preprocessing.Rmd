...
dim.create("data/derived/metabolomics/QCd",     rec=T)
...
fwrite(sample_info, "data/derived/metabolomics/sample_info.csv")
gt(head(sample_info))
```
...
met_info <- rbindlist(fill=T, list(
  ...
  fread(aligned_csvs$AN, select=1:10)
))
...
fwrite(met_info, "data/derived/metabolomics/met_info.csv")
gt(head(met_info))
```
...
data <- list()
data$CP <- fread(aligned_csvs$CP, drop=1: 9, colClasses='double') |> t() |> `colnames<-`(fread(aligned_csvs$CP, select=2)$Compound_ID)
data$CN <- fread(aligned_csvs$CN, drop=1: 9, colClasses='double') |> t() |> `colnames<-`(fread(aligned_csvs$CN, select=2)$Compound_ID)
data$HP <- fread(aligned_csvs$HP, drop=1: 9, colClasses='double') |> t() |> `colnames<-`(fread(aligned_csvs$HP, select=2)$Compound_ID)

# Have to handle a weird non-numeric string in the AN data
data$AN <- fread(aligned_csvs$AN, drop=1:10)
data$AN[, .SD, .SDcols=is.character][, lapply(.SD, \(x) grep('[^0-9.]', x, val=T))] # Look for non-numeric cells. There is one: "1+2559:2584.03935869994933"
data$AN[grepl('[^0-9.]', MESA__TOM148122), MESA__TOM148122 := NA]                   # Set to NA
data$AN <- data$AN[, names(.SD) := lapply(.SD, as.numeric)] |> t() |> `colnames<-`(fread(aligned_csvs$AN, select=2)$Compound_ID)

data <- lapply(data, \(m) m[grepl('TOM',rownames(m)),] ) # Rm QC samples
data$AN[data$AN<0] <- NA # Amines has 19 negative measurements, which don't make sense

Map(fwrite, data, paste0("data/derived/metabolomics/cleaned/",names(data),"_cleaned.csv"), row.names=T)
```
...
Map(fwrite, data, paste0('data/derived/metabolomics/QCd/',names(data),'_QCd.csv'))
```
...
fwrite(data_merged, 'data/derived/metabolomics/QCd/merged_QCd.csv')
```
