...
#load('baseline_added-251003.RData')
```
...
```{r, eval=T}
tmp2[by=.(lm_or_lmm,snp),
     , tmp2[lm_or_lmm==.BY['lm_or_lmm'] & snp==.BY['snp'] &
            ACME_p < `p_thresh_after_met~pa->met~hdl->hdl~met*snp`[lm_or_lmm==.BY['lm_or_lmm'] & snp==.BY['snp'],thresh] &
            met %in% runs[lm_or_lmm==.BY['lm_or_lmm'] & model=='hdl~met*snp' & signif_funnel==T, met]
            , signif_funnel := T
]] |> invisible()

tmp2[by=.(lm_or_lmm,snp),
     , tmp2[lm_or_lmm==.BY['lm_or_lmm'] & snp==.BY['snp'] &
            ACME_p < `p_thresh_after_hdl~met*snp`[lm_or_lmm==.BY['lm_or_lmm'] & snp==.BY['snp'],thresh] &
            met %in% runs[lm_or_lmm==.BY['lm_or_lmm'] & model=='hdl~met*snp' & signif_mwis==T, met]
            , signif_mwis := T
]] |> invisible()
```
```{r, eval=T}
runs     [model=='met~pa',      .(`met~pa`              =sum(signif_funnel,na.rm=T)), by=.(lm_or_lmm)
   ][on=.(lm_or_lmm),
     runs[model=='met~hdl',     .(`met~hdl`             =sum(signif_funnel,na.rm=T)), by=.(lm_or_lmm)]
   ][on=.(lm_or_lmm),
     runs[model=='hdl~met*snp', .(`hdl~met*snp (funnel)`=sum(signif_funnel,na.rm=T)), by=.(lm_or_lmm,snp)]
   ][on=.(lm_or_lmm,snp),
     tmp2[                    , .(`meMO (funnel)`       =sum(signif_funnel,na.rm=T)), by=.(lm_or_lmm,snp)]
   ][on=.(lm_or_lmm,snp),
     runs[model=='hdl~met*snp', .(`hdl~met*snp (MWIS)`  =sum(signif_mwis,  na.rm=T)), by=.(lm_or_lmm,snp)]
   ][on=.(lm_or_lmm,snp),
     tmp2[                    , .(`meMO (MWIS)`         =sum(signif_mwis,  na.rm=T)), by=.(lm_or_lmm,snp)]
   ][order(snp,-lm_or_lmm)
   ][grepl('14',snp)][,snp:=NULL
] |>
  #setcolorder(c('snp','lm_or_lmm')) |>
  #setnames(old=c('snp','lm_or_lmm'), new=c('SNP','Model')) |>
  setcolorder('lm_or_lmm') |>
  setnames(old='lm_or_lmm', new='Model') |>
  gt() |>
  tab_header('Number of significant metabolites after each step', 'with LM vs. LMM comparison')
```
