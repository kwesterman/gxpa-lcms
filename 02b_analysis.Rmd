...
] |> gt() |> tab_header('LMM Mediated Moderation Results', 'for metabolites significant from either GxPA MWIS or analysis funnel (the funnel ones are highlighted)') |>
  gt_highlight_rows(rows = met %in% runs[model=='hdl~met*snp' & signif_funnel, met])
```
...
runs     [model=='met~pa',      .(`met~pa`              =sum(signif_funnel,na.rm=T)), by=.(single_exam)
   ...
   # TODO: baseline/longitudinal sintead of lm lmm or singel_exam T/F
   ][grepl('14',snp)][,snp:=NULL
] |>
  #setcolorder(c('snp','single_exam')) |>
  #setnames(old=c('snp','single_exam'), new=c('SNP','Model')) |>
  setcolorder('single_exam') |>
  setnames(old='single_exam', new='Model') |>
  gt() |>
