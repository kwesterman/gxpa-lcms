...
```{r mediation, cache=T}
mediation_results <- mwas_gpc[signif==T][
  ][, med_res := mapply(met_names,vars2test,covar_sets,.I, FUN = \(met,var,covar_set,i) { # No mcmapply, mediation already multithreaded.
        data <- data[!is.na(hdl_log)]
        mediate(
          lm(data[[   met   ]] ~ ., data[,c(    var,covar_set),with=F]),
          lm(data[['hdl_log']] ~ ., data[,c(met,var,covar_set),with=F]),
          treat=var, mediator=met, robustSE=T, sims=100)
      }, SIMPLIFY=F)
...
```{r intrx, cache=T}
interaction_results <- mwas_gpc[signif==T][
