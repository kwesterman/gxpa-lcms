...
#load('mwas-250406.RData') # Comment this out to disable cheating mode
```
...
* __Mediation Model__: Comparing models: \
  `  met   ~       mvpa_wins + covars` and \
  `hdl_log ~ met + mvpa_wins + covars` \
* __Interaction Model__: \
  `hdl_log ~ met*SNP + sex*SNP + covars` \
* __Moderated Mediation__: \
  `  met   ~               mvpa_wins*SNP_bin + covars` and \
  `hdl_log ~ met*SNP_bin + mvpa_wins*SNP_bin + covars`
...
one_offs <- mclapply(one_offs, \(fmla)
  tryCatch( lm  (fmla,data), error=\(e) # TODO using tryCatch kinda goofy?
            lmer(fmla,data)
...
# Moving foward with only the nomically-significant variants, remove the others.
...
## Metabolite-PA association and Mediation models
```{r met-models}
mwas <- mwas[
...
```{r tbls1, eval=T, echo=F}
knitr::kable(mwas[, .(unique_met_id,lm_est,    lm_se,      lm_p,p_thresh_mwas   )][order(          lm_p)][1:3], caption = paste('Significant metabolites, met ~ PA MWAS (LM) :', mwas[,sum(          lm_p<p_thresh_mwas     ,na.rm=T)]))
knitr::kable(mwas[, .(unique_met_id,lmm_est,  lmm_se,     lmm_p,p_thresh_mwas   )][order(         lmm_p)][1:3], caption = paste('Significant metabolites, met ~ PA MWAS (LMM):', mwas[,sum(         lmm_p<p_thresh_mwas     ,na.rm=T)]))
knitr::kable(mwas[, .(unique_met_id,med_lm_prop,  med_lm_acme_p,p_thresh_med_lm )][order( med_lm_acme_p)][1:3], caption = paste('Significant metabolites, mediation     (LM) :', mwas[,sum( med_lm_acme_p<p_thresh_med_lm   ,na.rm=T)]))
knitr::kable(mwas[, .(unique_met_id,med_lmm_prop,med_lmm_acme_p,p_thresh_med_lmm)][order(med_lmm_acme_p)][1:3], caption = paste('Significant metabolites, mediation     (LMM):', mwas[,sum(med_lmm_acme_p<p_thresh_med_lmm  ,na.rm=T)]))
```
## GxE models:
