...
runs <- CJ(met=met_nms, snp=cpaids, single_exam=c(F,T), exam=1,
  ...
  # Finalize
        ...
        fifelse(grepl(   'C8_pos', fmla), '+ column_cp',
        fifelse(grepl(  'C18_neg', fmla), '+ column_cn', # Warning: code will silently break if you change the suffixes on metabolite names
        fifelse(grepl('HILIC_pos', fmla), '+ column_hp',
...
runs[model=='hdl~met*snp' & single_exam==F & grepl('chr14',snp) &
     #p < `p_thresh_after_met~pa->met~hdl`[single_exam==F,thresh] &
     met %in% runs[single_exam==F & model=='met~hdl' & signif_funnel==T, met]
     , p
]
...
totalruns[
    ...
    , ggplot(data=.SD,
        ...
        xlab('Cross-population β') + ylab('African-only β') +
...
#                ,subtitle='Model: log(hdl) ~ metabolite*SNP + covars') +
        xlab('Cross-population SE') + ylab('African-only SE') +
        #labs(color='One of the 71 GxMs\nsignificant in the\noriginal SNP\n(rs10146950)\n')
]
```
