...
one_offs[ grep('gxpa_lmm', names(one_offs)) ] |>                              # Get only GxPA LMM results
  ...
  (\(dt) dt[
    ...
    ][fread('variants_of_interest2.csv'), on='cpaid'                          # Merge w/ variant annotations
...
mwas <- mwas[, .SD, .SDcols=names(mwas)[!grepl('chr',names(mwas)) | grepl('chr2_121657822_C_T|chr14_88305056_G_A', names(mwas))]] # Drop cols referring to variants that are not the two nominally-significant ones. TODO make more readable
```
