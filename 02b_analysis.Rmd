...
#load('all_models_p_threshes-250814.RData') # Uncomment to cheat
```
...
runs <- runs[lm_or_lmm=='LMM'
  , c('est','se','df','t','p','singular_fit') := transpose(mcMap(fmla,term_pat, f=\(fmla,pat) {
      ...
      c(s$coefficients[grepl(pat,rownames(s$coefficients)),],              # Extract results
        grepl('singular', s$optinfo$conv$lme4$messages)     )              # Extract warning msgs
}))]
...
`p_thresh_after_met~pa->met~hdl->hdl~met*snp` <- 0.05/calc_eff_n_metabolites(`signif_mets_met~pa->met~hdl->hdl~met*snp`)
...
| `hdl~met_RESET`| `resettest(hdl_log ~ met_exam1 + covars)`                                        | RESET test p-value (LM, not LMM)
