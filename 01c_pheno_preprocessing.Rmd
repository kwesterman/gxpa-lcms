...
# Functions
```{r}
# Removes front matter and trailing backslashes from MESA phenotype names
# E.g. \\phs000209\\pht001116\\phv00084442\\age1c\\ -> age1c.
# The regex extracts what is between the last 2 sets of double-backslashes.
fix_pheno_names <- function(pheno_names) {
  unlist(lapply(pheno_names, function(nm) {
    if(grepl("phs000209", nm)) { str_match(nm, ".*\\\\(.*)\\\\$")[,2] } else {nm}
  }))
}
...
basic_info_df <- readxl::read_excel("phenotypes/draw_dates_MESA_study_site.xlsx") %>%
  ...
  select(mesa_id=sidno, gender1, race, site, month, season)
...
## Batch info
```{r}
# TODO sample_info.csv format is wack
batch_info_df2 <- read_csv("sample_info.csv", col_types=cols())
...
basic_pheno_df <- read_csv("phenotypes/basic_phenotypes.csv", col_types=cols()) %>%
    ...
    mutate(mesa_id = gsub("phs000209.v13_", "", `\\_Parent Study Accession with Subject ID\\`), # phs000209.v13_<id#> -> <id#>
...
nmr_lp3_df <- read_csv("phenotypes/nmr_metabolites.csv", col_types=cols()) %>%
    rename_with(fix_pheno_names, everything()) %>%
    mutate(mesa_id = gsub("phs000209.v13_", "", `\\_Parent Study Accession with Subject ID\\`),
           mesa_id = as.integer(mesa_id)) %>%
    select(
        mesa_id,
        HDL_C_lp3 = nhdlc31c,
...
```{r}
# TODO: DOSAGE FILE was read in here as a random RDS, replace with dosages.txt
# Samples as rows, rsids as cols
...
```{r}
system(paste0("gsutil cp -R $WORKSPACE_BUCKET/QCd ."))
```
...
```{r}
analysis_df_lcms <- analysis_df[match(rownames(mesa_metab_mat), analysis_df$TOM_ID), ]
```
