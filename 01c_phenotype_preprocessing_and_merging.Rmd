...
table(data$sex ) / sum(table(data$sex )) # Sex      proportions
#sapply(data, \(x) sum(is.na(x))) |> Filter(f=\(x) x!=0) # What cols have missing values, and how many
sapply(data[!duplicated(mesa_id), .(pa,mod_pa,mvpa,vig_pa,rs295849)], \(x) sum(!is.na(x))) # Sample sizes for each exposure/SNP
data[!duplicated(mesa_id), table(rs295849)] # Genotypes
```
