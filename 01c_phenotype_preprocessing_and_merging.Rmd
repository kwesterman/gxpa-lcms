...
Reduce(\(x,l) mergecoalesce(x, l$y, by=l$by, force_coalesce=T, all.x=T),
  list(                x=data,
    ...
    list(by='mesa_id', y=fread('phenotypes/id_match_file.csv')                   [, mesa_id := as.character(Cohort_Specific_Id)][!duplicated(mesa_id)]                  ), # Duplicated mesa_ids due to TOR id replicates. We aren't using RNAseq data so just pick whichever.
