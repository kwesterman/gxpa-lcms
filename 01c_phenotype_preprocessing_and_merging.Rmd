...
if(!file.exists('data/raw/phenotypes/')) {
  system('gcloud storage cp -r gs://fc-secure-4a392455-5587-4d6f-b8bd-01a1f834ae63/phenotypes data/raw')
...
# Join all datasets
```{r join}
# Create one dt containing all the IDs we could want.
...
mergecoalesce(by='mesa_id', all=T, force_coalesce=T,
  ...
  fread('data/raw/phenotypes/MESA_HDL_PA_covars_12345.csv')     [, mesa_id := `\\phs000209\\pht001116\\phv00084441\\sidno\\`]
...
Reduce(\(x,l) mergecoalesce(x, l$y, by=l$by, force_coalesce=T, all.x=T),
  list(                x=data,
    ...
    list(by= 'TOM_Id', y=fread('data/derived/metabolomics/sample_info.csv')               [, TOM_Id := sample_id]                                                                ),
    list(by= 'TOM_Id', y=fread('data/derived/metabolomics/QCd/merged_QCd.csv')                                                                                                   )) # Merge metabolomics last b/c it adds tons of cols
)
```
...
system('gcloud storage cp gs://fc-secure-c5905035-bf75-471b-8fa8-fb8a7f83c4a9/submissions/e81b3733-44cb-4c17-87db-2aee99a0491a/fetch_dbgap/d64cc7fd-47b7-4352-985f-3c3951b9c3cf/call-decrypt/glob-b3ddbe8d141590fbae0db21546878fa2/km.Rdata data/raw/genomics')
