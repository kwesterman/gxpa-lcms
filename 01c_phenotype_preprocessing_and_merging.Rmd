...
# Join all datqsets
```{r join}
mergecoalesce <- \(x,y,by,...) { # Merge which coalesces columns with duplicated names along the way
  setkeyv(x,by)
  setkeyv(y,by)
  merged <- merge(x,y, ...)

  basenames     <- sub('\\.x$|\\.y$', '', colnames(merged))
  basenames_dup <- basenames[duplicated(basenames)]

  for(basename in basenames_dup) {
    fullnames <- paste0(basename,c('.x','.y'))

    classes <- merged[, sapply(.SD,class), .SDcols=fullnames]
    if(!Reduce('==',classes)) {
      warning('Not coalescing these cols because their class doesn\'t match:\n',
              paste(capture.output(print(classes)), collapse='\n'))
    }

    merged <- merged[
        , basenames := fcoalesce(.SD), .SDcols= fullnames
      ][, .SD                        , .SDcols=!fullnames]
  }
...
data <- Reduce(\(x,l) mergecoalesce(x,l$y, by=l$by, all.x=T),
    list(              x=fread('metabolomics/QCd/merged_QCd.csv'),
    list(by= 'TOM_Id', y=fread('metabolomics/sample_info.csv')                          [, TOM_Id := sample_id]),
    list(by= 'TOM_Id', y=fread('phenotypes/MESA_PhenoUsmanSIDNO_20230214.txt')          [, TOM_Id := tomid1]),
    list(by= 'TOM_Id', y=fread('phenotypes/MESA_PhenoUsmanSIDNO_20230214.txt')          [, TOM_Id := tomid2]),
    list(by= 'TOM_Id', y=fread('phenotypes/MESA_PhenoUsmanSIDNO_20230214.txt')          [, TOM_Id := tomid3]),
    list(by= 'TOM_Id', y=fread('phenotypes/MESA_PhenoUsmanSIDNO_20230214.txt')          [, TOM_Id := tomid4]),
    list(by= 'TOM_Id', y=fread('phenotypes/MESA_PhenoUsmanSIDNO_20230214.txt')          [, TOM_Id := tomid5]),
