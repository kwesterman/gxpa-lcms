...
met_info[Method=='Amide-neg', unique_met_id := paste0(    .I,     '_',Method)] # Amines have no Compound_ID, so generate them.
...
data <- list()
data$CP <- fread(data_csvs$CP, skip=9, drop=1:8, colClasses='double') |> t() |> `colnames<-`(met_info[Method==   'C8-pos',unique_met_id]) # Yes I double-checked, the names are in the proper order
...
8174/12183 non-QC samples have metabolomics data for all of CP/CN/HP/AN. We would prefer to preserve those ~4k samples that don't, so we merge the metabolomics data with an outer join. TODO better words
```{r merge, message=F}
