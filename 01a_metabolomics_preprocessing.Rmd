...
if(!file.exists("metabolomics/raw")) system(paste0("gsutil -m cp -r ",ws_bucket,"/metabolomics .")) 
...
mode(data$C8) <- mode(data$HP) <- mode(data$AN) <- "numeric" # "NAs introduced by coersion" are due to 2 weird cells in AN: "1+2559:2584.03935869994933" and "5+3828:3930.95273887971489"

data <- lapply(data, \(m) m[!(rownames(m) %in% sample_info[is_qc_sample==T,sample_id]), ] ) # Rm QC samples
...
```
```{r results="hold"}
data$C8[1:3,1:3] # Check out the cleaned data!
data$HP[1:3,1:3]
data$AN[1:3,1:3]
```
...
mapply(fwrite, data, paste0("metabolomics/QCd/",names(data),"_QCd.csv"), row.names=T)
```


# Merge C8/HP/AN datasets
```{r message=F}
sample_ids_shared <- Reduce(intersect, lapply(data, rownames))
#all(grepl("TOM",sample_ids_shared)) # No QC samples!
data_merged <- do.call(cbind, lapply(data, \(m) m[sample_ids_shared,]))
fwrite(data_merged, "metabolomics/QCd/merged_QCd.csv", row.names=T)
```
---

# Plots
Plots are made using the C8/HP/AN data separately (not merged).
