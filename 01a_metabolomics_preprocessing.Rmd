...
sapply(c("metabolomics/cleaned","metabolomics/QCd","phenotypes"), dir.create, recursive=T)
```
...
fwrite(sample_info, "phenotypes/sample_info.csv")
head(sample_info)
```
...
invisible(mapply(fwrite, dts, paste0("metabolomics/cleaned/",names(dts),"_cleaned.csv"), row.names=T))
...
invisible(mapply(fwrite, dts, paste0("metabolomics/QCd/",names(dts),"_QCd_log2.csv"), row.names=T))
```
