...
trunc_inds <- \(v, bounds=quantile(v,c(0.05,0.95),na.rm=T)) v>bounds[1] & v<bounds[2]

rbind(data[,                 .(hdl,        mvpa , f='identity') ],
      data[trunc_inds(mvpa), .(hdl,        mvpa ,   'trunc'   ) ],
  ...
  facet_wrap(vars(f), scales='free')
```
## Sensitivity analysis on variable transformations
TODO
```{r}
