...
rbind(data[, .(sex, hdl, mvpa,           f='identity'   ) ],
      data[, .(sex, hdl, mvpa_resid,       'resid'      ) ],
      data[, .(sex, hdl, mvpa_log,         'log'        ) ],
      data[, .(sex, hdl, mvpa_log_resid,   'resid_log'  ) ],
      data[, .(sex, hdl, mvpa_wins,        'winsor'     ) ],
      data[, .(sex, hdl, mvpa_wins_resid,  'resid_wins' ) ],
      data[, .(sex, hdl, mvpa_trunc,       'trunc'      ) ],
      data[, .(sex, hdl, mvpa_trunc_resid, 'resid_trunc') ],
      use.names=F)[
    ][, f := factor(f,levels=unique(f)) ] |>
  ggplot(aes(x=mvpa, y=hdl)) +
  geom_smooth(aes(color=sex), method = 'gam', formula = y ~ s(x, bs='cs'), se=T, na.rm=T) +
...
```
```{r hist-quant, cache=T, cache.extra=digest::digest(data4hists_quant), message=F, warning=F}
