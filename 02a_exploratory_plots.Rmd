...
data[
  ][, .(bmi, sex, pa, mod_pa, mvpa, vig_pa, age, ses_score, drinks_per_week, ahei_score, dash_score, rs295849)
...
data <- data[
  ...
  ][, mvpa_resid_nosex       := resid(lm(mvpa       ~ ., data[, .SD, .SDcols=setdiff(covar_sets$`+gPCs`,'sex')], na.action=na.exclude))
  ][, mvpa_log_resid_nosex   := resid(lm(mvpa_log   ~ ., data[, .SD, .SDcols=setdiff(covar_sets$`+gPCs`,'sex')], na.action=na.exclude))
  ][, mvpa_wins_resid_nosex  := resid(lm(mvpa_wins  ~ ., data[, .SD, .SDcols=setdiff(covar_sets$`+gPCs`,'sex')], na.action=na.exclude))
  ][, mvpa_trunc_resid_nosex := resid(lm(mvpa_trunc ~ ., data[, .SD, .SDcols=setdiff(covar_sets$`+gPCs`,'sex')], na.action=na.exclude))
]

rbind(data[, .(sex, hdl, d1=mvpa,            d2=mvpa,                 f='identity'   ) ],
      data[, .(sex, hdl,    mvpa_resid,         mvpa_resid_nosex,       'resid'      ) ],
      data[, .(sex, hdl,    mvpa_log,           mvpa_log,               'log'        ) ],
      data[, .(sex, hdl,    mvpa_log_resid,     mvpa_log_resid_nosex,   'resid_log'  ) ],
      data[, .(sex, hdl,    mvpa_wins,          mvpa_wins,              'winsor'     ) ],
      data[, .(sex, hdl,    mvpa_wins_resid,    mvpa_wins_resid_nosex,  'resid_wins' ) ],
      data[, .(sex, hdl,    mvpa_trunc,         mvpa_trunc,             'trunc'      ) ],
      data[, .(sex, hdl,    mvpa_trunc_resid,   mvpa_trunc_resid_nosex, 'resid_trunc') ],
      use.names=F)[
...
vars2hist_quant <- c('bmi','pa','mod_pa','mvpa','vig_pa','age','ses_score','drinks_per_week','ahei_score','dash_score','hdl','hdl_log')
...
data4hists_categ <- melt(data[, c('sex', ..vars2hist_categ)],
  ...
  value.name    = 'val'
    ...
    'Missing', '< $5,000', '$5,000 - 7,999', '$8,000 - 11,999', '$12,000 - 15,999', '$16,000 - 19,999', '$20,000 - 24,999', '$25,000 - 29,999', '$30,000 - 34,999', '$35,000 - 39,999', '$40,000 - 49,999', '$50,000 - 74,999', '$75,000 - 99,999', '$100,000 +'
))]
```
```{r hist-quant, cache=T, cache.extra=digest::digest(data4hists_quant), message=F, warning=F, fig.width=16, fig.height=8}
ggplot(data4hists_quant, aes(x=val, fill=sex)) + 
  geom_density(alpha=0.5) +
  facet_wrap(vars(var), scales = 'free') +
  geom_vline(aes(xintercept= means, color=sex)) +
  geom_vline(aes(xintercept=`-1sd`, color=sex), linetype='dashed') +
  geom_vline(aes(xintercept=`+1sd`, color=sex), linetype='dashed')
```
```{r hist-categ, cache=T, cache.depend=digest::digest(data4hists_categ), message=F, warning=F}
